        ...
        <div id="left">
        	<div id="leftmenu">
                ...
                <ul>
                    ...
                    <a href="#">图说河中</a> -->
                    {pc:content action="category" catid="$firstcatid"}
                    <?php
                    ...
                    {pc:content action="category" catid="$catid" order="listorder ASC"}
                        {loop $data $v}
                            ...
                            {/if}
                        {/loop}
                    {/pc}<!--category:获取栏目的信息，lists:获取文章列表的信息，listorder:根据“排序”属性排序，通过loop让每一条数据循环-->
              </ul>
            </div>
          <div id="pic">
          <h1></h1>
...
<script type="text/javascript" src="{APP_PATH}api.php?op=count&id={$id}&modelid={$modelid}"></script><!--获取点击量三步走-->
</html>
